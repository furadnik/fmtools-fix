...
// but are 1-based and in cardinality ordering (like the f.m. themselves, the
// first element = emptyset) when they contain all  m values of all interaction
// indices
{
  ...
  for (k = 0; k < K; k++) {
    ...
    for (i = 0; i < RowsC1; i++) {
      row[2 + i + n + 1] =
          min_subset(&(XYData[k * (n + 1)]), n, card2bit[i + 1 + n]);
  ...
  for (A = n + 1; A < m; A++) {
    ...
    for (i = 0; i < n; i++)
      if (IsInSet(C, i)) {
        k = 1;
        for (B = 1; B <= C; B++)
          if (IsInSet(B, i) && IsSubset(C, B)) {
            if (card[B] == 1) {
              row[k] = 1;
              rowno[k] = (int)(bit2card[B] +
                               RowsR); // no need for +1, as it is 1-based
              k++;
            } else if (card[B] <= Kadd) {
              row[k] = 1;
        ...
        // finished loop for all B \subseteq A, now add the entries
        // corresponding to -1
        k1 = k;
        for (i1 = 1; i1 < k1; i1++)
          if (rowno[i1] > RowsR + n) {
  ...
  case 1:  // shapley lower bounds supplied
    ...
    if (options == 1)
      break;
  case 2: // shapley upper bounds supplied // almost the same as above, but
          // change of sign
    if (indexhigh != NULL)
      for (i = 0; i < n; i++)
        if (indexhigh[i] < 1) {
  ...
  case 4: // all lower bounds on interaction indices
    if (indexlow != NULL)
      for (A = 1; A < m; A++)
        if (indexlow[A] > -1) {
          ...
          k = 1;
          for (B = C; B < m; B++)
            if (IsSubset(B, C) && (card[B] <= Kadd)) { // why it ws C++ ???
    ...
    if (options == 4)
      break;
  case 5: // all upper bounds on interaction indices  // almost the same as
          // above
    if (indexhigh != NULL)
      for (A = 1; A < m; A++)
        if (indexhigh[A] < 1) {
  ...
  wei /= 2.;
  if ((option1 & 0x1) ==
      0x1) { // orness specified orness[0]=lower bound, orness[1]=upper bound
    if (orness[0] > 0) {
      row[0] = orness[0] * wei;
      rowno[0] = 0;
      k = 1;
  ...
  if ((option1 & 0x2) ==
      0x2) { // balanced. Means there plenty of conditions of the same type as
             // monotonicity constraints, but more of those
             // this is not yet implemented, reserved for future use
  }
  if ((option1 & 0x4) ==
      0x4) { // presevation of output orderings. to reduce the number of
             // conditions, sort the outputs in increasing order
    tempyk = new valindex[K];

    for (k = 0; k < K; k++) {
      (tempyk[k]).v = XYData[k * (n + 1) + n];
      (tempyk[k]).i = k;
    }
    ...
    for (i1 = 0; i1 < K - 1; i1++) {
      ...
      for (k1 = 0; k1 < RowsC1; k1++) {
        row[k] = min_subset(&(XYData[j * (n + 1)]), n, card2bit[k1 + n + 1]) -
                 min_subset(&(XYData[i * (n + 1)]), n, card2bit[k1 + 1 + n]);
        rowno[k] = RowsR + k;
  ...
  if (res == OPTIMAL) {
    ...
    for (i = 0; i < RowsC1; i++) {
      v[i + n + 1] = sol[n + RowsR + 1 + i] -
                     sol[n + RowsR + 1 + RowsC1 + i]; // other subsets
      k++;
    }
    k = n + RowsC1 + 1;
    for (unsigned int ii = k; ii < m; ii++)
      v[ii] = 0; // all other values of f.m. are 0
    result = 1;
  } // no optimal
  else
    result = 0;
...

{
  ...
  for (k = 0; k < K; k++) {
    ...
    for (i = 0; i < n; i++) {
      (tempxi[i]).v = XYData[k * (n + 1) + i];
      (tempxi[i]).i = i;
    }
    if (!(options == 1)) {
      sort(&(tempxi[0]), &(tempxi[n]),
           less_than1); // sorted in increasing order
...
double max_subset_complement(
    double *x, int n,
    int_64 S) { // returns min x_i when i \in S, or 0 if S is empty
  int i;
  double r = -10e10;
  for (i = 0; i < n; i++)
    if (!IsInSet(S, i))
...
// indexlow, indexhigh are 0-based for Shapley values (contain only singletos
// but are 1-based and in cardinality ordering (like the f.m. themselves, the
// first element = emptyset) not needed!! when they contain all  m values of all
// interaction indices
{
  ...
  Kadd--; // because m(Kadd)=1, no need those variables /// check this in
          // debugger
  ...
  for (k = 0; k < (unsigned int)K; k++) {
    ...
    for (A = RowsC + 1; A < m; A++)
      row[0] -= maxf(0, min_subset(&(XYData[k * (n + 1)]), n, card2bit[A]) -
    ...
    for (i = 0; i < (unsigned int)n; i++) { // singletons
      row[2 + i + 1] = maxf(0, XYData[k * (n + 1) + i] -
                                   max_subset_complement(&(XYData[k * (n + 1)]),
                                                         n, card2bit[i + 1]));
      rowno[2 + i + 1] = RowsR + i + 1;
    }
  ...
  if ((option1 & 0x4) ==
    ...
    for (i1 = 0; i1 < (unsigned int)K - 1; i1++) {
      ...
      for (k1 = 0; k1 < RowsC1; k1++) {
        row[k] = min_subset(&(XYData[j * (n + 1)]), n, card2bit[k1 + n + 1]) -
                 min_subset(&(XYData[i * (n + 1)]), n,
                            card2bit[k1 + 1 + n]); /// here change to g_A
        rowno[k] = RowsR + k;
  ...
  if (res == OPTIMAL) {
    ...
    k = n + RowsC1;
    for (unsigned int ii = k; ii < m; ii++)
      v[ii] = 1; //// all other values of f.m. are 1
    result = 1;
  } // no optimal
  else
    result = 0;
...
// interaction indices
{
  ...
  for (i = n; i > (unsigned int)Kadd; i--) { /*numberC[i] = i*Binomial(n, i); */
    numberCtotal += i * Binomial(n, i);      /*numberC[i]; */
  }

  RowsC1 =
      (cardpos[n] - n - 1); // how many non-singletons   ///Check this mu(0)=0.
                            // Also fewer variables as all are positive
  RowsR = K * 2;
  RowsC = n + RowsC1 + numberCtotal; /// removed *2
  ...
  for (k = 0; k < (unsigned int)K; k++) {
    ...
    row[0] = XYData[k * (n + 1) + n]; // y[k]; //
    /// minus sum of g_A over all subsets whose m(A)=1, at the very least m(N)=1
    //		for (A = n + RowsC1 + 1; A<m; A++) row[0] -= maxf(0,
    //min_subset(&(XYData[k*(n + 1)]), n, card2bit[A]) -
    //max_subset_complement(&(XYData[k*(n + 1)]), n, card2bit[A]));  ///?????
  ...
  row[0] = 0;
  rowno[0] = 0;
  for (A = (unsigned int)n + 1; A <= (unsigned int)n + RowsC1;
       A++) { /// check if we need <=
    //	cout<<"start subset  "<< A<<endl;
    C = card2bit[A];
    for (i = 0; i < (unsigned int)n; i++)
  ...
  for (A = RowsC2; A <= (unsigned int)n + RowsC1; A++) { /// check if we need <=
    ...
    C = card2bit[A];
    for (i = 0, j = 0; i < (unsigned int)n; i++)
      if (IsInSet(C, i)) {
        k = 1;
        row[k] = 1;
        rowno[k] = int(A + RowsR);
  ...
  row[0] = 1;
  rowno[0] = 0;
  for (A = (unsigned int)n + RowsC1; A <= (unsigned int)n + RowsC1;
       A++) { /// check if we need <=1
    k = 1;
    row[k] = 1;
    rowno[k] = int(A + RowsR);
    k++;
  ...
  */
  ...
  if (res == OPTIMAL || res == SUBOPTIMAL || res == TIMEOUT ||
      res == PROCBREAK || res == FEASFOUND) {
    //	temp = 0;
    get_primal_solution(MyLP, sol);

    //		for (i = 0; i < RowsC - (n + RowsC1) ; i++)
    //			printf("\n%d, %f ", i, sol[i + RowsR + RR + 1 + RowsC1 +
    //n]);
    ...
    for (i = 0; i < RowsC1; i++) {
      v[i + n + 1] = sol[n + RowsR + 1 + i +
                         RR]; ////- sol[n+RowsR+1 + RowsC1+i]; // other subsets
      k++;
    }
    k = n + RowsC1 + 1;
    for (unsigned int ii = k; ii < m; ii++)
...
*/
  ...
  if (!MIP) {
    ...
    for (A = RowsC2; A <= (unsigned int)n + RowsC1;
         A++) { /// check if we need <=
      //	cout<<"start subset  "<< A<<endl;
      minA = 1000;
      minAV = 10e20;
      ...
      for (i = 0, j = 0; i < (unsigned int)n; i++)
        if (IsInSet(C, i)) {
          ...
          if (sol[r1 + RowsR + RR + 1 + RowsC1 + n] < minAV) {
            minA = r1;
            minAV = sol[r1 + RowsR + RR + 1 + RowsC1 + n];
          }
          r1++;
        }
