...
using namespace std;
// #include <R.h>
//  note that programs using this code should be linked against lp_solve
//  library, which should be downloaded and installed separatly. These
//  references are only to the readers of that library.
...
template <typename Container> struct compare_indirect_index {
...
class greaterindex {
public:
  const bytee *container;
  int N = 0;

  greaterindex(const bytee *incontainer, const int n) {
...
bytee string2number(string &s, bytee i) { return (bytee)s[i] - 1; }
void number2string(string &s, bytee i, bytee n) { s[i] = n + 1; }

void Setinsert(int_64 &S, string St, bytee pos) {
  AddToSet(&S, string2number(St, pos));
}
void Setremove(int_64 &S, string St, bytee pos) {
  RemoveFromSet(&S, string2number(St, pos));
}
...

{
  ...
  RowsC1 = (cardpos[Kadd] - n -
            1); // how many non-singletons   ///Check this mu(0)=0. Also fewer
                // variables as all are positive
  RowsR = K * 2;
  RowsC = n + RowsC1; /// removed *2
  unsigned int RowsC2 =
      (cardpos[Kadd - 1]); // position of the second last cardinality

  MyLP = make_lp(RowsR + RowsC, 0);
  MyLP->do_presolve = FALSE;
  set_verbose(MyLP, 0);
  unsigned int itemp =
      RowsC + 2 + 1; // just the max number of entries per column
  ...
  for (k = 0; k < (unsigned int)K; k++) {
    ...
    sort(ind, ind + n,
         compare_indirect_index<double *>(
             &(XYData[k * (n + 1)]))); // decreasing order now
    ...
    for (i = 0; i < (unsigned int)n; i++) { // singletons
      row[2 + itemp + 1] =
          maxf(0, XYData[k * (n + 1) + i] -
                      max_subset_complement(&(XYData[k * (n + 1)]), n,
                                            card2bit[i + 1]));
    ...
    for (i = 0; i < RowsC1; i++) {
      row[2 + itemp + 1] = maxf(
          0, min_subset(&(XYData[k * (n + 1)]), n, card2bit[i + 1 + n]) -
                 max_subset_complement(
                     &(XYData[k * (n + 1)]), n,
                     card2bit[i + 1 + n])); /// new function g_A, need
                                            /// max_subset_complement, also need
                                            /// to move 1s to the RHS with y
      ...
      //			rowno[2+i+n + RowsC1+1] = RowsR + n + i +1  +
      // RowsC1;
    }
  ...
  for (A = n + 1; A <= RowsC; A++) { /// check if we need <=
    ...
    for (i = 0; i < (unsigned int)n; i++)
      if (IsInSet(C, i)) { /// check if inequality is reverse
        row[0] = 0;
        rowno[0] = 0;
        k = 1;
        row[k] = 1;
  ...
  if (res == OPTIMAL) {
    ...
    for (i = 0; i < RowsC1; i++) {
      v[i + n + 1] = sol[n + RowsR + 1 +
                         i]; ////- sol[n+RowsR+1 + RowsC1+i]; // other subsets
      k++;
    }
    k = n + RowsC1 + 1;
...
void AddSetToVars(Mymap &MyMap, int_64 &A, int s, unsigned int &j,
                  unsigned int &k) {
  // existing A
...
void AddPairSetsToVars(Mymap &MyMap, int_64 &A, int_64 &B, unsigned int &j,
                       unsigned int &k) {
  // existing A
  //	int b = int(MyMap.size());
...
void AddMonotonicityConstraints(int n, int_64 m, int K, int Kadd, Mymap &MyMap,
                                set<std::string> &MC, int *index, string &S,
                                double KConst, set<pair<int, int>> &monindices,
  ...
  L3:;
    ...
    ; // already in the set, no constraints are needed
}
...
void ProcessConstraints(int n, int_64 m, int K, int Kadd, Mymap &MyMap,
                        set<std::string> &MC, string &S, double KConst,
  ...
  for (int i = 1; i < Kadd; i++) {
    ...
    for (multimap<int, int_64>::iterator it = range.first; it != range.second;
         ++it) {
      auto Ai = MyMap.find(mtokey(it->second, 0));
      for (k = 1; k <= Kadd - i; k++) {
        auto ran1 = CardSet.equal_range(i + k);
        for (multimap<int, int_64>::iterator it1 = ran1.first;
             it1 != ran1.second; ++it1) {
...
// fr now no other paprameters are used
{
  ...
  RowsR = K * 2;
  RowsC = n + RowsC1; /// removed *2
  //	unsigned int RowsC2 = (cardpos[Kadd - 1]); // position of the second
  // last cardinality
  unsigned int RowsC3 =
      (cardpos[Kadd]); // position of the second last cardinality
  ...
  for (k = 0; k < (unsigned int)K; k++) {
    ...
    myiota(ind, ind + n, 0); // found in <numeric>
    sort(ind, ind + n,
         compare_indirect_index<double *>(&(XYData[k * (n + 1)])));

    AddMonotonicityConstraints(n, m, K * 2 + 1, Kadd, MyMap, MC, ind, S, KConst,
                               monindices, CardSet, SetChain);
    if (n - Kadd - 1 >= 0)
      for (i = 0; i < (unsigned int)(n - Kadd - 1); i++) {
        row[0] -= factor * ((&(XYData[k * (n + 1)]))[ind[n - i - 1]]);
    ...
    // for (A = RowsC + 1; A<m; A++) row[0] -= maxf(0, min_subset(&(XYData[k*(n
    // + 1)]), n, card2bit[A]) - max_subset_complement(&(XYData[k*(n + 1)]), n,
    // card2bit[A]));  ///?????
    ...
    for (i = 0; i < (unsigned int)Kadd; i++) {
      ...
      temp = (&(XYData[k * (n + 1)]))[ind[i]] -
             ((i + 1 < (unsigned int)n) ? (&(XYData[k * (n + 1)]))[ind[i + 1]]
                                        : 0);
  ...
  ProcessConstraints(n, m, K * 2 + 1, Kadd, MyMap, MC, S, KConst, monindices,
                     CardSet, SetChain);
  ...
  //	MyLP = make_lp(RowsR + RowsC, 0); ///??????? w dont know the number of
  // variables yet. reserve or not? 	MyLP->do_presolve = FALSE;
  // set_verbose(MyLP, 3);
  ...
  double wei = K; // this constraint is taken with that weight, to ensure it is
                  // actually satisfied
  if (wei < 1)
    wei = 1;
  ...
  if (res == OPTIMAL) {
    ...
    for (i = 1; i < RowsC3; i++) {
      ...
      if (Ai != MyMap.end()) {
        ...
        v[i] =
            sol[Ai->second + RowsR + 1 +
                RR]; // in row mode we add RR, for dual, we don't
                     //				cout << Ai->second+RowsR+1 <<"
                     //"<<Ai->first
                     //<<" "<<v[i]<< endl;
      } else
        v[i] = 0;

      {
        ...
        for (j = (card[A] >= 2) ? (cardpos[card[A] - 2]) : 0;
             j < (unsigned int)cardpos[card[A] - 1]; ++j) {
          B = card2bit[j];
          if (IsSubset(A, B))
            v[i] = maxf(v[i], v[j]);
          // v[i] = maxf(v[i], v[j]);
    ...
    /*
    this method will work for "compressed" FM representation to be done in the
    future. For now expand it into all FM values
...
*/
void AddVarsConstraints(int n, int_64 m, int Kadd, Mymap &MyMap, int *index,
                        double KConst, vector<double> &coefs,
                        vector<int_64> &indices, vector<int_64> &indices1,
                        double *X) {
  unsigned int j, k;
  int_64 A = 0;
  ...
  for (j = 0; j <= (unsigned int)Kadd; j++) {
    ...
    int b = int(MyMap.size());
    auto t =
        MyMap.insert(pair<int_64, int>(mtokey(A, index[j]), b)); // marginal
    if (t.second) // new var inserted
      k = b;
    else
      k = (t.first)->second; // already in the set of variables
...
void process_constraint_recursive(lprec *MyLP, int *rowno, double *row,
                                  int_64 B, int level, int shift, int Kadd,
                                  set<pair<int, int>> &modindices, int parent,
  ...
  if (level == 0) {
    ...
    if (parent >= 0)
      modindices.insert(pair<int, int>(parent, rowno[level]));
  ...
  for (int j = 0, i = 0; i <= level; i++) {
    ...
    process_constraint_recursive(MyLP, rowno, row, bit2cardm[A], level - 1,
                                 shift, Kadd, modindices, rowno[level], bound1,
                                 bound2);
  }
}
void process_constraint_recursive(lprec *MyLP, int *rowno, double *row,
...
void process_constraint_start(lprec *MyLP, int n, int Kadd, int shift,
                              double KConst, int *rowno, double *row, int low,
                              int up, set<pair<int, int>> &modindices,
                              double bound1, double bound2) {
  int k = 0;
  row[0] = KConst;
  rowno[0] = 0;
  ...
  while (k < up - low) {
    for (int i = 0; i < 1; i++) {
      process_constraint_recursive(MyLP, rowno, row, low + k + i, Kadd - 1,
                                   shift, Kadd, modindices, -1, bound1, bound2);
    }
    k += Kadd;
  }
}
...
int DeterminePos(int_64 C, int_64 B, int_64 A, int card) {
  ...
  for (int i = 0; i < card; i++) {
    ...
    // A = setfromkey(card2bitm[B + i]); // this can be done once only at the
    // start
    AddToSet(&C1, k);
    // if (prev == k) return i;
    if (C1 == A)
...
int FuzzyMeasureFitLPKinteractiveMarginal(int n, int_64 m, int K, int Kadd,
  ...
  if (option1 == 1 && KConst > (Kadd + 1.) / n)
    KConst = (Kadd + 1.) / n;
  if (option1 == 2 && KConst < (Kadd + 1.) / n)
    KConst = (Kadd + 1.) / n;

  RowsR = K * 2; // RowsC = n + RowsC1;  /// removed *2

  unsigned int RowsC3 =
      (cardposm[Kadd + 1]); // position of the  last cardinality
  ...
  if (option1 == 2) // sub/supermodularity constraints
    process_constraint_start(MyLP, n, Kadd + 1, (2 * K), KConst, rowno, row,
                             cardposm[Kadd], cardposm[Kadd + 1], modindices,
                             factor, 1.0); // sub
  else if (option1 == 1)
    process_constraint_start(MyLP, n, Kadd + 1, (2 * K), KConst, rowno, row,
                             cardposm[Kadd], cardposm[Kadd + 1], modindices,
  ...
  if (res == OPTIMAL) {
    ...
    //		print_solution(MyLP, 1);
    //			for (i = 0; i < RR + 1 + CC; i++) cout << sol[i] <<
    // endl;
    ...
    for (int_64 ii = cardpos[Kadd]; ii < m; ii++) {
      ...
      if (Kadd < n - 1)
        v[card2bit[ii]] =
            KConst + (j - Kadd - 1.0) * (1. - KConst) / (n - Kadd - 1);
      else
        v[card2bit[ii]] = 1;
    }
...
int FuzzyMeasureFitLPKinteractiveMarginalMaxChain(
  ...
  //	unsigned int itemp = RowsC + 2 + 1; // just the max number of entries
  // per column
  ...
  for (k = 0; k < (unsigned int)K; k++) {
    ...
    AddVarsConstraints(n, m, Kadd, MyMap, ind, KConst, coefs, indices, indices1,
                       (&(XYData[k * (n + 1)])));
  }
  ...
  for (k = 0; k < (unsigned int)K; k++) {
    ...
    for (i = 0; i <= (unsigned int)Kadd; i++) {
      ...
      if (option1 != 0) {
        if (parent >= 0)
          modindices.insert(pair<int, int>(parent, rowno[1 + i]));
        parent = rowno[1 + i];
        if (i == (unsigned int)Kadd)
          set_bounds(MyLP, rowno[1 + i], bound1, bound2);
      }
    ...
    row[0] = 1;
    rowno[0] = int(indices1[2 * Kadd + j + 1] + 2 * K + 1);
    ...
    for (i = 1; i < RowsC3; i++) {
        ...
        v[i] =
            sol[Ai->second + RowsR + 1 +
                RR]; // in row mode we add RR, for dual, we don't
                     //				cout << Ai->second + RowsR + 1
                     //<< " "
                     //<< (setfromkey(Ai->first)) << " " << v[i] << endl;

        //				cout << v[i] << " \t{" << ShowValue(A)
        //<<
        //"}"
        //<< " \t(" << Ai->second + RowsR + 1  << ")" << endl;
      } else
        v[i] = 0;
      ...
      if (1) {
        ...
        for (j = (card[A] >= 2) ? (cardpos[card[A] - 2]) : 0;
          ...
          if (IsSubset(A, B))
            v[i] = maxf(v[i], v[j]);
          // it looks this is only for superadditive, it should be the other way
          // around for subadditive. Postpone for now.
        }
      }
...
double SolveLP(lprec *MyLP, double KConst, int Kadd, int n, int K, int CC,
               int RowsR, int RowsC2, int RowsC, double *Coefs1, double *Coefs2,
               double *Coefs3) {
  double factor = (1.0 - KConst) / (n - Kadd - 1);
...

{
  ...
  *KConst1 = KConst = 0.1;
  ...
  if (Kadd == n - 1)
    goto Lcont; // no need to solve, the only solution is KConst=1

  delete_lp(MyLP);
  MyLP = copy_lp(MyLPSave);
  // setup new LP
  KConst = alf1;
  falf1 = minval = SolveLP(MyLP, KConst, Kadd, n, K, CC, RowsR, RowsC2, RowsC,
  ...
  // setup new LP
  KConst = alf2;
  falf2 = minval = SolveLP(MyLP, KConst, Kadd, n, K, CC, RowsR, RowsC2, RowsC,
                           Coefs1, Coefs2, Coefs3);

  for (i = 0; i < (unsigned int)maxiter; i++) {

    //	cout << i << endl;
    if (fabs(alf1 - alf2) < tol)
      goto GoldenExit;
    ...
    if (falf2 > falf1) {
      ...
      MyLP = copy_lp(MyLPSave);
      KConst = alf1;
      falf1 = minval = SolveLP(MyLP, KConst, Kadd, n, K, CC, RowsR, RowsC2,
                               RowsC, Coefs1, Coefs2, Coefs3);
    }
    ...
    // otherwise, use the right hand interval
      ...
      MyLP = copy_lp(MyLPSave);
      KConst = alf2;
      falf2 = minval = SolveLP(MyLP, KConst, Kadd, n, K, CC, RowsR, RowsC2,
                               RowsC, Coefs1, Coefs2, Coefs3);
...
GoldenExit:
  ...
  *KConst1 = KConst;
  ...
  while (1){
          ...
          minval= SolveLP(MyLP,  KConst,  Kadd,  n,  K,  CC,  RowsR,  RowsC2,
RowsC,  Coefs1, Coefs2,  Coefs3); if (minval<minsave){ minsave = minval;
KConstSave = KConst; }
